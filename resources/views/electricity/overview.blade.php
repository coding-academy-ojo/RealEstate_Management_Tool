...
@section('title', 'Electricity Services Overview')
...
@section('content')
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
        <div>
            <h2 class="mb-1">
                ...
                Electricity Services Overview
                <small class="text-muted fs-6 fw-normal" style="font-family: 'Segoe UI', Tahoma, sans-serif;">
                    لوحة معلومات الكهرباء
                </small>
            </h2>
            <p class="text-muted mb-0">
                Monitor calculated electricity consumption, solar performance, and billing health
                <span class="text-muted small d-block mt-1">تتبع استهلاك الكهرباء المحسوب وأداء الطاقة الشمسية
                    والفواتير</span>
            </p>
        </div>
        <div class="d-flex flex-wrap gap-2">
            ...
            <a href="{{ route('electricity.companies.index') }}" class="btn btn-outline-orange">
    ...
    @php
        ...
        $cards = [
            ...
            [
                ...
                'meta' => 'Calculated readings / 12m',
                'meta_ar' => 'قراءات محسوبة خلال 12 شهراً',
                'trend' => '+9.1%',
                'trend_positive' => true,
            ],
            [
            ...
            [
                ...
                'meta' => 'Per reading (calculated)',
                'meta_ar' => 'لكل قراءة محسوبة',
            ],
            [
                'label' => 'Inactive + Trash',
                'label_ar' => 'خدمات متوقفة أو محذوفة',
                'value' => number_format(
                    ($summaryStats['inactive_services'] ?? 0) + ($summaryStats['soft_deleted_services'] ?? 0),
                ),
                'icon' => 'bi-pause-circle',
                'icon_bg' => 'bg-secondary-subtle',
                'icon_color' => 'text-secondary',
                'meta' => $inactiveRatio . '% inactive / ' . $softDeletedRatio . '% trash',
                'meta_ar' => 'نسب الخروج من الخدمة',
            ],
        ];
    @endphp
    ...
    <div class="row mb-4">
        @foreach ($cards as $card)
            <div class="col mb-3">
                <div class="card dashboard-card border-0 shadow-lg h-100" style="background-color: #111827 !important;">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1 ms-3">
                                <h6 class="mb-0 text-uppercase small fw-bold"
                                    style="color: {{ $card['icon_color'] === 'text-success' ? '#198754' : ($card['icon_color'] === 'text-primary' ? '#0d6efd' : ($card['icon_color'] === 'text-danger' ? '#dc3545' : ($card['icon_color'] === 'text-warning' ? '#ffc107' : '#6c757d'))) }};">
                                    {{ $card['label'] }}
                                </h6>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h5 class="mb-0 fw-bold text-dark">
                                ...
                                Calculated Consumption Trend
                            </h5>
                            <small class="text-muted" style="font-family: 'Segoe UI', sans-serif;">اتجاه استهلاك الكهرباء
                                المحسوب - آخر 12 شهراً</small>
                        </div>
                        <span class="badge bg-warning text-dark">12 Months</span>
                    </div>
                </div>
                <div class="card-body">
                    @if (array_sum($consumptionTrend['series']) > 0)
                        <canvas id="electricConsumptionTrendChart" height="260"></canvas>
                    @else
                        <div class="text-center text-muted py-5">
                            ...
                            <p class="small mb-0" style="font-family: 'Segoe UI', sans-serif;">لا توجد بيانات استهلاك</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card border-0 shadow-lg h-100">
                ...
                <div class="card-body">
                    @if (array_sum($companyBreakdown['series']) > 0)
                        <canvas id="electricCompanyDistributionChart" height="260"></canvas>
                    @else
                        <div class="text-center text-muted py-5">
                            <i class="bi bi-pie-chart" style="font-size: 3rem; opacity: 0.3;"></i>
                            <p class="mt-3 mb-1 fw-semibold">No company data</p>
                            <p class="small mb-0" style="font-family: 'Segoe UI', sans-serif;">أضف شركات الكهرباء لرؤية
                                التوزيع</p>
                        </div>
                    @endif
                </div>
            ...
            <div class="card border-0 shadow-lg h-100">
                ...
                <div class="card-body">
                    @if (array_sum($governorateBreakdown['series']) > 0)
                        <canvas id="electricGovernorateDistributionChart" height="320"></canvas>
                    @else
                        <div class="text-center text-muted py-5">
                    ...
                    <div class="table-responsive">
                        <table class="table mb-0 align-middle">
                            <thead class="table-light">
                                <tr>
                                    <th scope="col" class="ps-3">Service</th>
                                    <th scope="col">Company</th>
                                    <th scope="col" class="text-end pe-3">Amount</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="fw-semibold">
                                                {{ $service->electricityCompany?->name ?? ($service->company_name ?? 'N/A') }}
                                            </div>
                                            @if ($service->electricityCompany?->name_ar)
                                                <div class="text-muted small"
                                                    style="font-family: 'Segoe UI', sans-serif;">
                                                    {{ $service->electricityCompany->name_ar }}
                                                </div>
                                            @endif
                                        </td>
                                ...
                                @forelse ($recentReadings as $reading)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <div class="text-muted small">
                                                {{ $service?->electricityCompany?->name ?? ($service?->company_name ?? 'Unknown') }}
                                            </div>
                                        </td>
                                        <td>
                                            <span class="fw-bold text-info">{{ number_format($consumption, 2) }}</span>
                                            <small class="text-muted">kWh</small>
                                            <div class="text-muted small" style="font-size: 0.75rem;">
                                                Imported calc:
                                                {{ number_format((float) ($reading->imported_calculated ?? 0), 2) }}
                                            </div>
                                        </td>
                                        <td>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const chartColors = [
                '#f59e0b', '#0d6efd', '#10b981', '#ef4444', '#6366f1',
                '#14b8a6', '#f97316', '#8b5cf6', '#ec4899', '#6b7280'
            ];
            ...
            const consumptionCtx = document.getElementById('electricConsumptionTrendChart');
                            ...
                            tooltip: {
                                ...
                                callbacks: {
                                    label: function(context) {
                                        return `Consumption: ${context.parsed.y.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kWh`;
                                    },
                                },
                            },
                        },
            ...
            const companyCtx = document.getElementById('electricCompanyDistributionChart');
            ...
            const governorateCtx = document.getElementById('electricGovernorateDistributionChart');
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            tooltip: {
                                ...
                                callbacks: {
                                    label: (context) => `Services: ${context.parsed.x.toLocaleString()}`,
                                },
                            },
                        },
                        scales: {
